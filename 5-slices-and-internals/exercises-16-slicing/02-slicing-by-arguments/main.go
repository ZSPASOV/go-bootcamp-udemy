...
func main() {
	...
	var positionsNum []int

	for _, pos := range positions {
		position, _ := strconv.Atoi(pos)
		positionsNum = append(positionsNum, position)
	}

	if len(positionsNum) != 1 && len(positionsNum) != 2 {
		fmt.Println("Provide only the [starting] and [stopping] positions")

		return
	}

	if len(positionsNum) > 1 {
		if (positionsNum[0] > positionsNum[1]) || positionsNum[0] > len(ships) || positionsNum[1] > len(ships) {
	...
	if len(positionsNum) == 1 {
		sliced := ships[positionsNum[0]:]
		fmt.Println(sliced)
	}

	if len(positionsNum) == 2 {
		sliced := ships[positionsNum[0]:positionsNum[1]]
		fmt.Println(sliced)
	}
}
